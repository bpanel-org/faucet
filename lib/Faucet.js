...
class Faucet extends PureComponent {
  ...
  async onSubmit() {
    try {
      ...
      const rate = await this.client.node.execute('estimatefee', [10]);
  ...
  render() {
    ...
    const { network, chain } = this.props;
    const currency = new Currency(chain, balance);
    return (
      <div>
        ...
        <Text type="p">
          Faucet Balance: {currency.to('currency')}{' '}
          {currency.getUnit('unit').toUpperCase()}
        </Text>
        <Text type="p">
        ...
        {receiveAddress.length ? (
          <Text type="condensed" copyIcon={true} prefix={20} suffix={20}>
            {receiveAddress}
          </Text>
        ) : (
          <Text type="p">
